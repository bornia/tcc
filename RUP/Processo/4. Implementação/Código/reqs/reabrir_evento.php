...
if($usuario_permissao != 3) {
	ob_clean();
	echo '<strong>Apenas o Dono </strong> do grupo pode reabrir um evento. Peça para o Dono <u>lhe conceder acesso</u> ou que <u>ele mesmo reabra o(s) evento(s)</u>.';
	return false;
}

$sql = "UPDATE eventos SET status = 1 WHERE evento_id IN ($eventos_id);";
...
if(!$res) {
	ob_clean();
	echo '<strong>Houve um erro ao tentar reabrir o(s) evento(s) do grupo</strong>. <u>Por favor, contate o suporte urgentemente</u>.';
	return false;
}
